...
# datos para la derivada meridional
cos2 = (np.cos(np.deg2rad(yd)))**2
cos1 = np.cos(np.deg2rad(yd))
...
for k in range (0,nz):
    ...
    dy =  ( -1.0/(R*cos2) ) * ( np.gradient(flux,axis=1)/np.gradient(yd,axis=0) )
    #print ()
...
os.system("rm diffFluxcamip.nc")
# abriendo el archivo netcdf
f = Dataset('diffFluxcamip.nc','w', format='NETCDF4') #'w' stands for write
